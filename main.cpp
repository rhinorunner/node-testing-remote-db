...
{	
	...
	// set up remote database information
	sql::mySQL_info dbInfo {
		"tcp://127.0.0.1:3306",
		"87331401500196",
		"localDbStorage.txt",
		80, false
	};
	sql::Connector dbConnector {&dbInfo};
	if (!dbConnector.open_connection(false)) 
		// database cannot be connected to
		throw std::invalid_argument("Remote database cannot be connected to.");
	else std::cout << "Database connected.\n";
	dbConnector.refresh_connection();
	...
	while (looping)
	{
		...
		else if (event.type == SDL_KEYDOWN)
		{
			...
			case SDLK_q:
  				dbConnector.upd_connection(0,pathConv_str(nodeConnections[0]));
				std::cout << "!! saved path 0\n";
				break;
			
			case SDLK_a:
				OVERRIDE[0] = pathConv_vec(dbConnector.get_connection(0));
				std::cout << "!! displaying path 0\n";
				break;
			
			case SDLK_z:
				OVERRIDE[0] = {};
				std::cout << "!! undisplaying path 0\n";
				break;
			}
		}
		...
		for (uint16_t B = 0; B < nodeConnections.size(); ++B) {
			...
			if (nodeShow[B]) {
				auto toShow = overr ? OVERRIDE[B] : nodeConnections[B];
				for (uint16_t j = 0; j < toShow.size(); ++j) {
					// make sure the node isnt the last element
					if (j == (toShow.size() - 1)) break;

					uint16_t nodeIndex = findId(toShow[j]);

					Node temp1 = N_NODES[findId(toShow[j])];
					Node temp2 = N_NODES[findId(toShow[j+1])];
